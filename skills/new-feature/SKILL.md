...
**반드시 Bash `printf`로 출력**한다. 모델 텍스트 출력은 Claude Code 렌더러가
ANSI escape를 무력화하므로 색상이 보이지 않는다. Bash tool의 stdout 경로만
색상이 살아남는다.

렌더링 패턴, 색상 팔레트, 볼드 선정 규칙, 상태별 변형은
[references/templates.md](references/templates.md)의
"작업 완료 보고 — 터미널 렌더링 가이드" 섹션을 따른다.

**원칙:**
- Phase 1~3 및 4.1의 정보 가공 단계는 모두 기존대로 유지
- Bash printf는 **최종 사용자 노출 순간 한 번만** 사용
